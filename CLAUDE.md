...
> **Known component gap:** `PhotoBand` — the 4-image horizontal band used on `freedom250.html` — is the one live section type with no component yet (it wasn't present in the july4v2 / corporate / newdam ports the kit was built from). It touches none of the three templates. Build it as a prop-driven component **if/when freedom250 is migrated to Astro**, not before.
...
**The one exception — the public homepage (`/`):** it has no code step, so its visits are recorded in [middleware.js](middleware.js) instead, *before* the gate logic and **passing through** (the homepage is never gated). Because it's public there's no code to wash out bots, so each hit is classified by **User-Agent**: real visitors get a timestamped, **geo-only** event (deliberately **no IP** — avoids the PII/retention burden on a full-history log), while suspected bots only bump a visible `imari:home:bots` tally (nothing silently dropped — the heuristic is imperfect, so the tally is the audit). This is **homepage-only**; the gated-page recording above is unchanged. The write is backgrounded via middleware's `event.waitUntil`.
